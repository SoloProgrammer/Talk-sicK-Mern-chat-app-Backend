    ...
    io.on("connection", (socket) => {
      ...
      socket.on("typing", (room, user) => {
        ...
        if (typingInfo.length) {
          if (
            !typingInfo.map((tyI) => tyI.chatId).includes(room) ||
            !typingInfo.map((tyI) => tyI.user?._id).includes(user._id)
          ) {
      ...
      socket.on("stop typing", (room, user) => {
        ...
        typingInfo = typingInfo.filter((tyI) => {
          if (
            (tyI.chatId !== room && tyI.user._id !== user._id) ||
            (tyI.chatId === room && tyI.user._id !== user._id)
          )
            return tyI;
        });
      ...
      socket.on("new message", (newMessageRecieved) => {
        ...
        usersLeftTheGroupChat = newMessageRecieved?.chat?.leftFromGroup.map(
        ...
        chat.users.forEach((user) => {
          if (
            ...
            (newMessageRecieved?.chat?.isGroupchat &&
              usersLeftTheGroupChat.includes(String(user._id)))
          )
            return;

          socket
            .in(user._id)
            .emit("message recieved", newMessageRecieved, user);
        });
      });
      ...
      socket.on("group created", (createdBy, createdGrp) => {
        ...
        grpUsersIds.forEach((uId) => {
          uId !== createdBy._id &&
            socket.in(uId).emit("group created", createdGrp);
        });
      });
        ...
        chat.users
          ...
          .forEach((uId) => {
            String(userId) !== String(uId) &&
              socket.in(uId).emit("group photo updated", chat._id, newPhotoURL);
          });
      });
      ...
      socket.on("group name changed", (userId, chat, newName) => {
        ...
        chat.users
          ...
          .forEach((uId) => {
            String(userId) !== String(uId) &&
              socket.in(uId).emit("group name updated", chat._id, newName);
          });
      });
    });
  } catch (error) {
    console.log(
      "Some error occured while connecting with the socket.io ERROR: ",
      error
    );
  }
};
